...
/*
#define xx(f_, str) \
	(FILE*)f_ = fopen(str.c_str(), "wb+")
	
	xx(f_head, amp->getHeadPath());
	xx(f_src, amp->getSrcPath());
	xx(f_mk, amp->getMkPath());
	xx(f_test, amp->getTestPath());
#undef xx
*/
	...
	std::cout << "test_str : " << "\n" << test_str << std::endl 
		<< "f_test: " << f_test
		<< "\ntest_name: " << amp->getTestPath() << std::endl;
